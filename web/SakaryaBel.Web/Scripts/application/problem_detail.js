var problem_detail = (function ($) {
    ...
    pub.changeProblemStatus = function (el) {
        $.ajax({
            url: '/Problem/ChangeProblemStatus/',
            type: 'POST',
            dataType: 'json',
            data: { problemId: problemId, status: $(el).attr("data-id") },
            success: function (data) {
                console.log(data);
                $("#statusIcon").removeClass(statusIcon);
                $("#statusIcon").css("background-color", "");
                if ($(el).attr("data-id") == 0)
                    statusIcon = "btn-danger";
                else if ($(el).attr("data-id") == 2)
                    statusIcon = "btn-success";
                else if ($(el).attr("data-id") == 1)
                    statusIcon = "btn-info";
                else if ($(el).attr("data-id") == 4)
                    $("#statusIcon").css("background-color", "#A523A7");

                $("#statusIcon").addClass(statusIcon);
            },
            error: function (res) {
                console.log('changeProblemStatus Error in process.');
            }
        });
    }
