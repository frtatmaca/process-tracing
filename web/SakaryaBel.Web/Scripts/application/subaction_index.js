var subaction_index = (function ($) {

    var viewModel = function () {
        this.SubActionList = ko.observableArray();
    ...
    function subaction(ActionId, Name, Description, SubActionCount, StringCreatedDate, SubActionType, Number) {
        this.ActionId = ko.observable(ActionId);
        this.Name = ko.observable(Name);
        this.Description = ko.observable(Description);
        this.SubActionCount = ko.observable(SubActionCount);
        this.StringCreatedDate = ko.observable(StringCreatedDate);
        this.SubActionType = ko.observable(SubActionType);
        this.Number = ko.observable(Number);
    }

    function LoadFromServer(pageNum, url) {
        // blmsCommon.showLoading();        
        actionService.getSubActionLists(url,
            function (data) {
                ...
                bindList(data[0]);
                var totalNumber=data[3];
                console.log("total nubmer:" + totalNumber);
    ...
    function getAdvancedSearchFilters(skip, top) {
        var subActionName = $("#subActionName").val();
        var startDate = $(".startDate").val();
        var endDate = $(".endDate").val();

        return { "actionId": actionId, "subActionName": subActionName, "startDate": startDate, "endDate": endDate, "skip": skip, "top": top };
    }
    ...
    function bindList(data) {
        ...
        item.SubActionList.removeAll();
        for (var i = 0; i < results().length; i++) {
            item.SubActionList.push(
                new subaction(
                    results()[i].ActionId(),
                    results()[i].Name(),
    ...
    pub.init = function (refActionId) {
        ...
        jQuery('#subActionName').on('input propertychange paste', function () {
            if ($("#subActionName").val().length == 0 || $("#subActionName").val().length > 4) {
                LoadFromServer(1, getFilters(0, $("#tablePageSize").val(), isAdvanced));
            }
        });
    ...
    pub.SubActionList = function () {
        return item.SubActionList();
    };
