var message_index = (function ($) {
    ...
    var viewModel = function () {
        ...
        this.MessajeInfo = ko.observable();
        this.SenderEmail = ko.observable();
        this.SenderUserName = ko.observable();
        this.SenderWebsite = ko.observable();
    ...
    function message(MessageId, ActiveStatus, Body, CreatedDate, MessajeInfo, SenderEmail, SenderUserName, SenderWebsite) {
        this.MessageId = ko.observable(MessageId);
        this.ActiveStatus = ko.observable(ActiveStatus);
        this.Body = ko.observable(Body);
        this.CreatedDate = ko.observable(CreatedDate);
        this.MessajeInfo = ko.observable(MessajeInfo);
        this.SenderEmail = ko.observable(SenderEmail);
        this.SenderUserName = ko.observable(SenderUserName);
        this.SenderWebsite = ko.observable(SenderWebsite);
    }

    function LoadFromServer(pageNum, url) {
        // blmsCommon.showLoading();
        console.log(url);
        messageService.getMessageLists(url,
            function (data) {
        ...
        for (var i = 0; i < results().length; i++) {
            item.MessageList.push(
                new message(
                    ...
                    results()[i].Body().substring(0, 300),
                    results()[i].CreatedDate() != null ? moment.utc(results()[i].CreatedDate()).local().format('llll') : null,
                    results()[i].MessajeInfo(),
                    //moment.utc(results()[i].LoginDate()).local().format('lll'),
                    results()[i].SenderEmail(),
                    results()[i].SenderUserName(),
                    results()[i].SenderWebsite()
                )
            );
        }
    ...
    pub.messageStatusReplace = function (el) {
        var msgId = $(el).attr("data-Id");
        var msgStatus = $(el).attr("data-status");

        $.ajax({
            url: "/Message/MessageStatusReplace/",
            type: "POST",
            dataType: "json",
            data: { messageId: msgId, status: msgStatus },
            success: function (res) {
                if (res == "success") {
                    alert("Message statusu değiştirildi");
                } else {
                    console.log("message statusu değiştirilir iken bir hata ile karşılaşıldı.");
                }
            },
            error: function (res) {
